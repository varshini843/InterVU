// Real Activity Tracker for InterVU
...
function trackPageVisit() {
  ...
  if (currentPage === 'interview.html') {
    // Will be tracked when interview is completed
  } else if (currentPage === 'resume.html') {
    // Will be tracked when resume is saved
  } else if (currentPage === 'chatbot.html') {
    // Will be tracked when chat session starts
  }
}
...
function updateActivityDurations() {
  ...
  ['interviews', 'resumes', 'chats', 'logins'].forEach(type => {
    userActivities[type].forEach(activity => {
      ...
      if (activity.timestamp > sessionStorage.getItem('intervu_session_start_time')) {
        ...
        if (durationMinutes > activity.duration) {
          ...
          if (type === 'interviews' || type === 'chats') {
            userStats.totalPracticeTime += (durationMinutes - activity.duration);
          }
        }
      }
    });
